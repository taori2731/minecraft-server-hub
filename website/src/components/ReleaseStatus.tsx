...
export function ReleaseStatus() {
  return <section className="section release-section" id="updates" aria-labelledby="updates-title">
    <div className="release-heading"><SectionHeading id="updates-title" title="今できることと、まだできないこと" description="公開版の事実と条件付きの機能を分けて掲載します。" /><dl><div><dt>現在の版</dt><dd>{releaseStatus.version}</dd></div><div><dt>公開日</dt><dd>{releaseStatus.updatedAt}</dd></div><div><dt>配布</dt><dd>{releaseStatus.download}</dd></div></dl></div>
    ...
    <p className="download-status"><Download aria-hidden="true" />公開版0.4.3の配布資産とTauri更新署名を確認済みです。インストール済み0.4.2からの最終更新確認は継続中です。</p>
