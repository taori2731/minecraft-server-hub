...
const faqs = [
  ...
  { q: "アプリは自動更新されますか？", a: "署名された更新フィードを確認し、本人が版と更新内容を承認した場合だけ適用する仕組みです。サーバーが動作中のときは更新しません。0.4.4候補のlatest.json、隣接.sig、SHA-256を準備中です。0.4.3からの最終更新確認は未実施/継続中です。" },
  ...
  { q: "0.4.4は公開されていますか？", a: "まだ公開していません。0.4.4は公開前の候補としてソースとWebsiteを準備中で、GitHub Releaseへの公開、一般配布、インストール、更新確認は未実施です。0.4.3からの最終更新確認は未実施/継続中です。" },
];
...
export function FaqDownloadFooter() {
  return <>
    <section className="section faq-section" id="faq" aria-labelledby="faq-title"><div className="faq-heading"><CircleHelp aria-hidden="true" /><SectionHeading id="faq-title" title="よくある質問" description="公開前候補の状態、対応範囲、支援方針をまとめました。" /></div><div className="faq-list">{faqs.map(({ q, a }) => <details key={q}><summary>{q}</summary><p>{a}</p></details>)}</div></section>
    <section className="download-section" id="download" aria-labelledby="download-title"><MonitorDown aria-hidden="true" /><div className="download-copy"><h2 id="download-title">公開予定 0.4.4</h2><p>0.4.4は公開前の候補です。GitHub Releaseへの公開と一般配布はまだ実施していません。更新互換性のためWindowsインストーラー資産名は従来形式を維持しています。</p><dl className="download-facts"><div><dt>パッケージ</dt><dd><code>{releaseStatus.installerName}</code></dd></div><div><dt>SHA-256</dt><dd><code>{releaseStatus.sha256}</code></dd></div><div><dt>署名方式</dt><dd>{releaseStatus.signatureMethod}</dd></div><div><dt>Authenticode</dt><dd>{releaseStatus.authenticodeStatus}</dd></div></dl><p className="download-proof">これは公開前に記録した候補資産のSHA-256です。公開後に<span>SHA256SUMS.txt</span>、<code>.sig</code>、<code>latest.json</code>を取得して再照合します。<code>.sig</code>はTauri Updaterが検証する署名資産で、Windows Authenticode署名を意味しません。</p><p className="download-warning">公開後に実行する場合は、SmartScreenが警告を表示する可能性があります。公式GitHub ReleaseのURL、Windowsが表示する発行元情報、ファイル名、SHA-256を確認し、不一致や不明点があれば実行しないでください。</p><p className="download-safety">アンインストールだけで外部サーバーフォルダーやワールドを削除しません。削除・移動の前にサーバー、ワールド、設定、バックアップを確認してください。</p><div className="download-links" aria-label="0.4.4候補の検証リンク"><a href={releaseStatus.releaseUrl} target="_blank" rel="noreferrer">GitHub Release（公開後）</a><a href={releaseStatus.manifestUrl} target="_blank" rel="noreferrer">latest.json（公開後）</a><a href={releaseStatus.signatureUrl} target="_blank" rel="noreferrer">隣接.sig（公開後）</a><a href={releaseStatus.checksumUrl} target="_blank" rel="noreferrer">SHA256SUMS.txt（公開後）</a></div></div><a className="button button-primary" href={releaseStatus.installerUrl}>0.4.4候補を確認</a></section>
    <footer className="site-footer"><div className="footer-brand"><Brand /><p>Windows x64向けゲームサーバー管理アプリ。公開予定0.4.4の候補を案内しています。GitHub公開前です。</p></div><div className="pending-links" aria-label="公式情報">{pendingLinks.map(([Icon, label]) => <span key={label}><Icon aria-hidden="true" />{label}<small>整備中</small></span>)}</div><p className="legal-note">Minecraft、Mojang Studios、Microsoft、Palworld、Pocketpair、Valveの公式製品・提携製品ではありません。TomoNodeは各社の承認・提携を受けていない独立プロジェクトです。</p><p className="copyright">© 2026 TomoNode</p></footer>
  </>;
}
