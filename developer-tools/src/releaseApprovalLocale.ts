...
const copies: Record<Locale, ReleaseApprovalCopy> = {
  en: {
    kicker: "Controlled release approval", title: "Release candidate approval gate", intro: "Rebuild the D20 evidence snapshot, evaluate every required gate, and append an explicit approval to a tamper-evident local ledger. Approval never builds, signs, uploads, or publishes anything.", privacy: "Reviewer and rationale stay in this app's local approval ledger. The source workspace is not modified.", nativeOnly: `Release approval is available only in the installed Windows ${developerBrand.productName} app.`,
  ...
  ja: {
    kicker: "管理されたリリース承認", title: "リリース候補の承認ゲート", intro: "D20証跡を再構築して必須条件を評価し、明示承認を改ざん検出付きのローカル台帳へ追記します。ビルド、署名、アップロード、公開は実行しません。", privacy: "承認者と理由はこのアプリのローカル承認台帳だけに保存され、ソースのワークスペースは変更しません。", nativeOnly: `リリース承認はインストール済みWindows版${developerBrand.productName}でのみ利用できます。`,
  ...
  "zh-CN": {
    kicker: "受控发布审批", title: "候选版本审批门", intro: "重新生成D20证据快照，评估全部必需条件，并将明确审批追加到可检测篡改的本地账本。不会构建、签名、上传或发布任何内容。", privacy: "审批人和理由仅保存在本应用的本地审批账本中，不会修改源工作区。", nativeOnly: `发布审批仅在已安装的Windows ${developerBrand.productName}应用中可用。`,
  ...
  "zh-TW": {
    kicker: "受控發行核准", title: "發行候選版本核准閘門", intro: "重新建立D20證據快照、評估所有必要條件，並將明確核准追加至可偵測竄改的本機帳本。不會建置、簽署、上傳或發佈任何內容。", privacy: "核准人與理由只保存在本應用程式的本機核准帳本，不會修改來源工作區。", nativeOnly: `發行核准僅能在已安裝的Windows ${developerBrand.productName}應用程式中使用。`,
  ...
  ko: {
    kicker: "통제된 릴리스 승인", title: "릴리스 후보 승인 게이트", intro: "D20 증거 스냅샷을 다시 만들고 모든 필수 조건을 평가한 뒤 명시적 승인을 변조 감지 로컬 원장에 추가합니다. 빌드, 서명, 업로드 또는 게시를 실행하지 않습니다.", privacy: "승인자와 사유는 이 앱의 로컬 승인 원장에만 저장되며 소스 작업 공간은 변경되지 않습니다.", nativeOnly: `릴리스 승인은 설치된 Windows ${developerBrand.productName} 앱에서만 사용할 수 있습니다.`,
  ...
  fr: {
    kicker: "Approbation de version contrôlée", title: "Porte d’approbation de la version candidate", intro: "Reconstruit la preuve D20, évalue toutes les conditions obligatoires et ajoute une approbation explicite à un registre local détectant les altérations. Rien n’est compilé, signé, téléversé ou publié.", privacy: "L’approbateur et la justification restent dans le registre local de cette application. L’espace de travail source n’est pas modifié.", nativeOnly: `L’approbation est disponible uniquement dans l’application Windows ${developerBrand.productName} installée.`,
