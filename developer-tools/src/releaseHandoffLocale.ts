...
const copies: Record<Locale, ReleaseHandoffCopy> = {
  ...
  ja: {
    title: "リリース引き継ぎパック", intro: "現在のD20証跡と、それに完全一致するD21承認アンカーを公開担当へ渡せる形にまとめます。内部変更は自己整合性で検出し、送信元の確認には信頼済みSHA-256または元のローカル台帳を使います。", privacy: "承認者名と承認理由は含めません。ハッシュ、警告識別子、日時、匿名化済みD20証跡だけを引き継ぎます。", nativeOnly: `リリース引き継ぎはインストール済みWindows版${developerBrand.productName}でのみ利用できます。`,
  ...
  "zh-CN": {
    title: "发布交接包", intro: "将当前 D20 证据与完全匹配的 D21 审批锚点打包交给发布人员。内部变更由自身完整性检查发现；确认来源需要可信 SHA-256 或原始本地台账。", privacy: "不包含审批者姓名或理由，仅传递哈希、警告标识、时间和已净化的 D20 证据。", nativeOnly: `发布交接仅可在已安装的 Windows ${developerBrand.productName} 中使用。`,
  ...
  "zh-TW": {
    title: "發行交接套件", intro: "將目前 D20 證據與完全相符的 D21 核准錨點封裝給發行人員。內部變更由自身完整性檢查發現；確認來源需要可信 SHA-256 或原始本機台帳。", privacy: "不包含核准者姓名或理由，只傳遞雜湊、警告識別碼、時間與已清理的 D20 證據。", nativeOnly: `發行交接僅能在已安裝的 Windows ${developerBrand.productName} 中使用。`,
  ...
  ko: {
    title: "릴리스 인계 팩", intro: "현재 D20 증거와 정확히 일치하는 D21 승인 앵커를 릴리스 담당자에게 전달합니다. 내부 변경은 자체 무결성으로 감지하며 출처 확인에는 신뢰한 SHA-256 또는 원본 로컬 원장을 사용합니다.", privacy: "승인자 이름과 사유는 제외하며 해시, 경고 식별자, 시간 및 정제된 D20 증거만 전달합니다.", nativeOnly: `릴리스 인계는 설치된 Windows ${developerBrand.productName}에서만 사용할 수 있습니다.`,
  ...
  "pt-BR": {
    title: "Pacote de transferência da versão", intro: "Agrupa a evidência D20 e sua âncora D21 exata. A autointegridade detecta mudanças; confirmar a origem exige um SHA-256 confiável ou o registro local original.", privacy: "O nome e a justificativa do aprovador são excluídos. Apenas hashes, alertas, datas e a evidência D20 sanitizada são transferidos.", nativeOnly: `A transferência está disponível somente no ${developerBrand.productName} instalado no Windows.`,
