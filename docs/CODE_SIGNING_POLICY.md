...
The developer-tools installer has its own configuration and feed. It is not included in the main application's 0.4.1 release gate.

## Roles

- Committers and reviewers: [`taori2731`](https://github.com/taori2731), the repository owner.
- Release approver: [`taori2731`](https://github.com/taori2731), responsible for approving the exact 0.4.1 candidate after all pre-public checks pass.
...
No secret value may be committed, printed, placed in an artifact, included in `latest.json`, or pasted into an issue or chat. If the existing private key is missing or does not produce a signature accepted by the embedded public key, the release stops. A replacement key must not be generated for 0.4.1.

## Required release flow

The manually dispatched `Release Windows updater` workflow accepts only `main`, `version=0.4.1`, and `release_tag=v0.4.1`. It:
...
9. refuses to reuse an existing release or tag and creates `v0.4.1` as a Draft;
...
The public key and the Tauri configuration key must not change for 0.4.1. This preserves the ability of installed 0.3.9 and 0.4.0 applications to validate the 0.4.1 update. The 0.3.9 and 0.4.0 Releases and their assets are never deleted or overwritten by the workflow.
