...
Memory Storeは開発・隔離試験専用です。ホスト、ホストごとのサーバー、招待、参加者、セッション、snapshot、操作結果と共有レート制限に上限があります。新規保存時に期限切れ・失効済みデータと保持期間を過ぎた終端操作を回収し、安全な回収対象がない場合は`relay-memory-capacity`で停止します。実行中の操作、未期限の参加者、未失効の招待、接続中ホストは容量確保のために削除しません。本番環境では`MSH_CO_MANAGEMENT_DATABASE_URL`を必須にし、Memory Storeへフォールバックしないでください。

RustホストのUIイベントキューには512件の上限があります。満杯または許可サイズを超えるイベントを受信した場合はイベントを破棄せずWSS接続を切断し、設定操作を成功扱いにしません。切断後は通常の再接続・結果照会・必要に応じたホスト側復旧の手順へ進みます。
...
- WSSは接続数、事前認証数、upgrade試行数、受信メッセージ数/バイト数、ホストイベントキューの件数/バイト数/ホスト数/TTL、接続済みソケットの送信バッファ上限をアプリ側でも管理する。送信バッファ超過時はホスト接続を閉じて送信を拒否し、リレーのプロセスバッファを無制限に増やさない。Cloudflare側のIPレート制限は別途設定・検証する
