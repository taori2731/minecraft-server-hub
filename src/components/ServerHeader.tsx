  ...
  return (
    <section className={`server-hero${compact ? " compact" : ""}`}>
      ...
      <div className="server-actions" aria-label={t("serverActions")}>
        <button type="button" className="action-button start" disabled={running || Boolean(busyAction)} onClick={onStart}><Icon name="play" />{busyAction === "start" ? t("starting") : t("start")}</button>
        <button type="button" className="action-button stop" disabled={!running || Boolean(busyAction)} onClick={onStop}><Icon name="stop" />{busyAction === "stop" ? t("stopping") : t("stop")}</button>
        <button type="button" className="action-button restart" disabled={!running || Boolean(busyAction)} onClick={onRestart}><Icon name="refresh" />{busyAction === "restart" ? t("restarting") : t("restart")}</button>
      </div>
    </section>
  );
}
