...
export const backend = {
  isDesktop: inDesktop,
  getAppVersion: () => inDesktop ? import("@tauri-apps/api/app").then(({ getVersion }) => getVersion()) : Promise.resolve("0.4.4"),
  quitApp: () => desktopOr<void>("quit_app", {}, () => undefined),
  checkAppUpdate: (endpoint?: string) => desktopOr<AppUpdateInfo>("check_app_update", { endpoint: endpoint?.trim() || null }, () => ({ configured: true, currentVersion: "0.4.4", available: false })),
