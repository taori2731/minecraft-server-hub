...
import type {
  ...
  CoManagementJournalMigrationResult,
  CoManagementOperationResult,
  CoManagementParticipant,
  CoManagementParticipantInput,
  CoManagementServerSnapshot,
  CoManagementSettingsInput,
  CoManagementSettingsSnapshot,
  ConfigureCoManagementInput,
...
const demoCoConfigs = new Map<string, CoManagementConfigView>();
const demoCoParticipants = new Map<string, CoManagementParticipant[]>([
  ["demo-paper", [{ id: "demo-participant", displayName: "Takeru", role: "editor", state: "pending", expiresAt: new Date(Date.now() + 8 * 60_000).toISOString(), createdAt: new Date().toISOString() }]],
]);
const demoCoInvites = new Map<string, CoManagementInviteResult[]>();
const demoCoOperations = new Map<string, CoManagementOperationResult>();
const demoCoAudit = new Map<string, CoManagementAuditEntry[]>();

function coGameKind(server: ServerProfile): string {
  if (isPalworldServer(server)) return "palworld";
  return server.serverType === "bedrock" ? "bedrock" : "java";
}

function coCapabilities(server: ServerProfile): CoManagementCapability[] {
  const stopped = true;
  if (isPalworldServer(server)) return [
    { key: "serverDescription", valueType: "string", editableWhenStopped: stopped, max: 256 },
    { key: "maxPlayers", valueType: "integer", editableWhenStopped: stopped, min: 1, max: 32 },
    { key: "expRate", valueType: "number", editableWhenStopped: stopped, min: 0.1, max: 20 },
    { key: "collectionDropRate", valueType: "number", editableWhenStopped: stopped, min: 0.1, max: 20 },
    { key: "palCaptureRate", valueType: "number", editableWhenStopped: stopped, min: 0.1, max: 20 },
    { key: "dayTimeSpeedRate", valueType: "number", editableWhenStopped: stopped, min: 0.1, max: 5 },
    { key: "nightTimeSpeedRate", valueType: "number", editableWhenStopped: stopped, min: 0.1, max: 5 },
    { key: "palEggDefaultHatchingTime", valueType: "number", editableWhenStopped: stopped, min: 0, max: 240 },
    { key: "deathPenalty", valueType: "string", editableWhenStopped: stopped, enumValues: ["None", "Item", "ItemAndEquipment", "All"] },
    { key: "invaderEnemiesEnabled", valueType: "boolean", editableWhenStopped: stopped },
    { key: "fastTravelEnabled", valueType: "boolean", editableWhenStopped: stopped },
    { key: "baseCampMaxNumInGuild", valueType: "integer", editableWhenStopped: stopped, min: 1, max: 20 },
    { key: "baseCampWorkerMaxNum", valueType: "integer", editableWhenStopped: stopped, min: 1, max: 50 },
  ];
  const common: CoManagementCapability[] = [
    { key: "difficulty", valueType: "string", editableWhenStopped: stopped, enumValues: ["peaceful", "easy", "normal", "hard"] },
    { key: "defaultGameMode", valueType: "string", editableWhenStopped: stopped, enumValues: ["survival", "creative", "adventure", "spectator"] },
    { key: "maxPlayers", valueType: "integer", editableWhenStopped: stopped, min: 1, max: 100 },
    { key: "viewDistance", valueType: "integer", editableWhenStopped: stopped, min: 2, max: 32 },
  ];
  if (server.serverType === "bedrock") return common;
  return [...common,
    { key: "pvp", valueType: "boolean", editableWhenStopped: stopped },
    { key: "allowFlight", valueType: "boolean", editableWhenStopped: stopped },
    { key: "forceGameMode", valueType: "boolean", editableWhenStopped: stopped },
    { key: "spawnProtection", valueType: "integer", editableWhenStopped: stopped, min: 0, max: 128 },
    { key: "simulationDistance", valueType: "integer", editableWhenStopped: stopped, min: 2, max: 32 },
  ];
}

function coFields(server: ServerProfile): Record<string, string | number | boolean> {
  if (isPalworldServer(server)) {
    const settings = server.palworldSettings ?? { serverDescription: "", maxPlayers: 1, restApiPort: 0, restApiEnabled: false, backupEnabled: false };
    return {
      serverDescription: settings.serverDescription,
      maxPlayers: settings.maxPlayers,
      expRate: settings.expRate ?? 1,
      collectionDropRate: settings.collectionDropRate ?? 1,
      palCaptureRate: settings.palCaptureRate ?? 1,
      dayTimeSpeedRate: settings.dayTimeSpeedRate ?? 1,
      nightTimeSpeedRate: settings.nightTimeSpeedRate ?? 1,
      palEggDefaultHatchingTime: settings.palEggDefaultHatchingTime ?? 72,
      deathPenalty: settings.deathPenalty ?? "All",
      invaderEnemiesEnabled: settings.invaderEnemiesEnabled ?? true,
      fastTravelEnabled: settings.fastTravelEnabled ?? true,
      baseCampMaxNumInGuild: settings.baseCampMaxNumInGuild ?? 3,
      baseCampWorkerMaxNum: settings.baseCampWorkerMaxNum ?? 15,
    };
  }
  ...
  if (server.serverType !== "bedrock") Object.assign(fields, {
    pvp: settings.pvp,
...
function demoCoSnapshot(serverId: string): CoManagementServerSnapshot {
  const server = demoServers.find((item) => item.id === serverId);
  if (!server) throw new Error("サーバーが見つかりません");
  const state = demoStates.get(serverId) ?? stoppedStatusForCo(server);
  const config = demoCoConfigs.get(serverId);
  return {
    serverId,
    serverName: server.name,
    gameKind: coGameKind(server),
    state: state.state,
    playerCount: state.playerCount,
    maxPlayers: state.maxPlayers,
    fetchedAt: new Date().toISOString(),
    revision: config?.revision ?? 1,
    capabilities: coCapabilities(server),
    enabled: config?.enabled ?? false,
    connectionState: config?.connectionState ?? "disconnected",
    participants: [...(demoCoParticipants.get(serverId) ?? [])],
    invites: [],
  };
}

function stoppedStatusForCo(server: ServerProfile): RuntimeStatus {
  return { state: "stopped", playerCount: 0, maxPlayers: getServerMaxPlayers(server), memoryUsedMib: 0, uptimeSeconds: 0, address: `localhost:${server.port}`, cpuPercent: 0, tps: null, tpsSupported: false, pingLatencyMs: null };
}

function addDemoCoAudit(serverId: string, action: string, changes: Record<string, unknown>, actorDisplayName = "ホストPC") {
  const entry: CoManagementAuditEntry = {
  ...
  demoCoAudit.set(serverId, [...(demoCoAudit.get(serverId) ?? []), entry]);
}

function applyDemoCoChanges(input: CoManagementApplyInput): CoManagementApplyResult {
  const server = demoServers.find((item) => item.id === input.serverId);
  if (!server) throw new Error("サーバーが見つかりません");
  const config = demoCoConfigs.get(input.serverId);
  if (!config?.enabled) throw new Error("共同管理が無効になっています");
  if (demoStates.get(input.serverId)?.state !== "stopped") throw new Error("サーバーが停止していないため、共同管理設定を保存できません");
  if (config.revision !== input.expectedRevision) throw new Error("409 Conflict: 設定revisionが古いため、再取得が必要です");
  const before = coFields(server);
  const capabilities = new Map(coCapabilities(server).map((item) => [item.key, item]));
  for (const [key, value] of Object.entries(input.changes)) {
    const capability = capabilities.get(key);
    if (!capability) throw new Error(`共同管理では「${key}」を変更できません`);
    if (capability.valueType === "boolean" && typeof value !== "boolean") throw new Error(`${key}の値が不正です`);
    const numericValue = typeof value === "number" ? value : Number.NaN;
    if (capability.valueType === "integer" && (!Number.isInteger(numericValue) || (capability.min !== undefined && numericValue < capability.min) || (capability.max !== undefined && numericValue > capability.max))) throw new Error(`${key}の値が不正です`);
    if (capability.valueType === "number" && (!Number.isFinite(numericValue) || (capability.min !== undefined && numericValue < capability.min) || (capability.max !== undefined && numericValue > capability.max))) throw new Error(`${key}の値が不正です`);
    if (capability.valueType === "string" && (typeof value !== "string" || (capability.enumValues && capability.enumValues.length > 0 && !capability.enumValues.includes(value)))) throw new Error(`${key}の値が不正です`);
  }
  if (isPalworldServer(server)) {
    server.palworldSettings = { ...(server.palworldSettings ?? { serverDescription: "", maxPlayers: 1, restApiPort: 0, restApiEnabled: false, backupEnabled: false }), ...input.changes };
  } else {
    server.settings = { ...server.settings, ...input.changes };
  }
  server.pendingRestart = true;
  server.updatedAt = new Date().toISOString();
  const after = coFields(server);
  const revision = config.revision + 1;
  demoCoConfigs.set(input.serverId, { ...config, revision, updatedAt: new Date().toISOString() });
  addDemoCoAudit(input.serverId, "settings.patch", { fields: Object.keys(input.changes), before, after });
  const result: CoManagementApplyResult = {
    requestId: input.requestId,
    serverId: input.serverId,
    revision,
    changedFields: Object.keys(input.changes).sort(),
    settings: {
      serverId: input.serverId,
      gameKind: coGameKind(server),
      state: "stopped",
    ...
    message: "ブラウザデモで設定変更を反映しました。実機では次回起動時に反映されます。",
  };
  demoCoOperations.set(input.requestId, { requestId: input.requestId, state: "completed", result, errorCode: undefined });
  return result;
}
...
export const backend = {
  isDesktop: inDesktop,
  getAppVersion: () => inDesktop ? import("@tauri-apps/api/app").then(({ getVersion }) => getVersion()) : Promise.resolve("0.3.2"),
  quitApp: () => desktopOr<void>("quit_app", {}, () => undefined),
  checkAppUpdate: (endpoint?: string) => desktopOr<AppUpdateInfo>("check_app_update", { endpoint: endpoint?.trim() || null }, () => ({ configured: true, currentVersion: "0.3.2", available: false })),
  ...
  coManagementSnapshot: (serverId: string) => desktopOr<CoManagementServerSnapshot>("get_co_management_snapshot", { serverId }, () => demoCoSnapshot(serverId)),
  coManagementConfig: (serverId: string) => desktopOr<CoManagementConfigView>("get_co_management_config", { serverId }, () => demoCoConfigs.get(serverId) ?? { serverId, enabled: false, endpoint: "http://127.0.0.1:8787", connectionState: "disconnected", revision: 1, permissionGeneration: 0, recoveryRequired: false, updatedAt: new Date().toISOString() }),
  coManagementLegacyJournalCount: (serverId: string) => desktopOr<number>("get_co_management_legacy_journal_count", { serverId }, () => 0),
  migrateCoManagementLegacyJournal: (serverId: string) => desktopOr<CoManagementJournalMigrationResult>("migrate_co_management_legacy_journal", { serverId, confirmation: "平文ジャーナルを暗号化して置換する" }, () => ({ serverId, migrated: 0, remainingLegacy: 0, databaseCompacted: true, message: "ブラウザデモでは旧形式ジャーナルはありません。" })),
  configureCoManagement: (input: ConfigureCoManagementInput) => desktopOr<CoManagementConfigView>("configure_co_management", { input }, () => {
    const server = demoServers.find((item) => item.id === input.serverId);
    if (!server) throw new Error("サーバーが見つかりません");
    const current = demoCoConfigs.get(input.serverId);
    const config: CoManagementConfigView = {
      serverId: input.serverId,
      enabled: input.enabled,
      endpoint: input.endpoint?.trim() || current?.endpoint || "http://127.0.0.1:8787",
      connectionState: input.enabled ? current?.connectionState === "connected" ? "connected" : "disconnected" : "disconnected",
      revision: current?.revision ?? 1,
      permissionGeneration: (current?.permissionGeneration ?? 0) + 1,
      recoveryRequired: current?.recoveryRequired ?? false,
      updatedAt: new Date().toISOString(),
    };
    demoCoConfigs.set(input.serverId, config);
    addDemoCoAudit(input.serverId, input.enabled ? "co-management.enable" : "co-management.disable", { enabled: input.enabled });
    return config;
  }),
  connectCoManagement: (serverId: string) => desktopOr<CoManagementConfigView>("connect_co_management", { serverId }, () => {
    const current = demoCoConfigs.get(serverId);
    if (!current?.enabled) throw new Error("共同管理を有効にしてから接続してください");
    const next = { ...current, connectionState: "connected", updatedAt: new Date().toISOString() };
    demoCoConfigs.set(serverId, next);
    return next;
  }),
  issueCoManagementInvite: (input: IssueCoManagementInviteInput) => desktopOr<CoManagementInviteResult>("issue_co_management_invite", { input }, () => {
    const config = demoCoConfigs.get(input.serverId);
    if (!config?.enabled) throw new Error("共同管理を有効にしてから招待を発行してください");
    const invite: CoManagementInviteResult = { inviteId: `demo-invite-${Date.now()}`, role: input.role, expiresAt: new Date(Date.now() + (input.expiresMinutes ?? 10) * 60_000).toISOString(), inviteUrl: `${config.endpoint ?? "http://127.0.0.1:8787"}/#invite=browser-demo-${crypto.randomUUID()}` };
    demoCoInvites.set(input.serverId, [...(demoCoInvites.get(input.serverId) ?? []), invite]);
    addDemoCoAudit(input.serverId, "invite.issue", { role: input.role, expiresAt: invite.expiresAt });
    return invite;
  }),
  pollCoManagementEvents: () => desktopOr<CoManagementEvent[]>("poll_co_management_events", {}, () => []),
  coManagementPendingCode: (input: CoManagementParticipantInput) => desktopOr<string>("get_co_management_pending_code", { input }, () => {
    const participant = (demoCoParticipants.get(input.serverId) ?? []).find((item) => item.id === input.participantId);
    if (!participant || participant.state !== "pending") throw new Error("共同管理の参加コードが期限切れです");
    return input.participantId === "demo-participant" ? "397218" : "------";
  }),
  approveCoManagementParticipant: (input: CoManagementParticipantInput) => desktopOr<CoManagementParticipantInput>("approve_co_management_participant", { input }, () => {
    const participants = demoCoParticipants.get(input.serverId) ?? [];
    demoCoParticipants.set(input.serverId, participants.map((item) => item.id === input.participantId ? { ...item, state: "approved", approvedAt: new Date().toISOString() } : item));
    addDemoCoAudit(input.serverId, "participant.approve", { participantId: input.participantId });
    return input;
  }),
  revokeCoManagementParticipant: (input: CoManagementParticipantInput) => desktopOr<CoManagementParticipantInput>("revoke_co_management_participant", { input }, () => {
    const participants = demoCoParticipants.get(input.serverId) ?? [];
    demoCoParticipants.set(input.serverId, participants.map((item) => item.id === input.participantId ? { ...item, state: "revoked" } : item));
    addDemoCoAudit(input.serverId, "participant.revoke", { participantId: input.participantId });
    return input;
  }),
  coManagementSettings: (input: CoManagementSettingsInput) => desktopOr<CoManagementSettingsSnapshot>("get_co_management_settings", { input }, () => {
    const server = demoServers.find((item) => item.id === input.serverId);
    if (!server) throw new Error("サーバーが見つかりません");
    const config = demoCoConfigs.get(input.serverId);
    return { serverId: input.serverId, gameKind: coGameKind(server), state: demoStates.get(input.serverId)?.state ?? "stopped", revision: config?.revision ?? 1, fetchedAt: new Date().toISOString(), fields: coFields(server), capabilities: coCapabilities(server), editable: Boolean(config?.enabled && demoStates.get(input.serverId)?.state === "stopped") };
  }),
  applyCoManagementSettings: (input: CoManagementApplyInput) => desktopOr<CoManagementApplyResult>("apply_co_management_settings", { input }, () => applyDemoCoChanges(input)),
  coManagementOperation: (serverId: string, participantId: string, requestId: string) => desktopOr<CoManagementOperationResult>("get_co_management_operation", { serverId, participantId, requestId }, () => demoCoOperations.get(requestId) ?? { requestId, state: "not-found" }),
  respondCoManagementRequest: (input: { serverId: string; requestId: string; ok: boolean; result?: unknown; errorCode?: string; errorMessage?: string }) => desktopOr<void>("respond_co_management_request", { input }, () => undefined),
  publishCoManagementSnapshot: (serverId: string) => desktopOr<void>("publish_co_management_snapshot", { serverId }, () => undefined),
  coManagementAudit: (serverId: string, participantId: string) => desktopOr<CoManagementAuditEntry[]>("get_co_management_audit", { serverId, participantId }, () => [...(demoCoAudit.get(serverId) ?? [])].reverse()),
